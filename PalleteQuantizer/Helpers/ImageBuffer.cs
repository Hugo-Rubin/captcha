...
using PalleteQuantizer.ColorCaches.Common;
using PalleteQuantizer.Ditherers;
using PalleteQuantizer.Extensions;
using PalleteQuantizer.PathProviders;
using PalleteQuantizer.Quantizers;

namespace PalleteQuantizer.Helpers
{
    public class ImageBuffer : IDisposable
    {
            ...
            Action<LineTask> processPerPixel = lineTask =>
                {
                    ...
                    for (Int32 pathOffset = lineTask.StartOffset; pathOffset < lineTask.EndOffset; pathOffset++)
                    {
                        ...
                        if (isAdvanced)
                        {
                            var processAdvancedFunction = (ProcessPixelAdvancedFunction) processingAction;
                            allowWrite = processAdvancedFunction(pixel, this);
                        }
                        else // use simplified version with pixel parameter only
                        {
                            var processFunction = (ProcessPixelFunction) processingAction;
            ...
            Action<LineTask> transformPerPixel = lineTask =>
                {
                    ...
                    for (Int32 pathOffset = lineTask.StartOffset; pathOffset < lineTask.EndOffset; pathOffset++)
                    {
                        ...
                        if (isAdvanced)
                        {
                            var transformAdvancedFunction = (TransformPixelAdvancedFunction) transformAction;
                        ...
                        {
                            var transformFunction = (TransformPixelFunction) transformAction;
                             ...
                             Int32 parallelTaskCount = 4)
        {
            ...
            List<Color> targetPalette = isTargetIndexed
            ...
            TransformPixelFunction calculateMeanError = (sourcePixel, targetPixel) =>
                {
                    ...
                    totalError += ColorModelHelper.GetColorEuclideanDistance(ColorModel.RedGreenBlue, sourceColor,
                                                                             targetColor);
                    return false;
                };
