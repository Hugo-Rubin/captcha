...
namespace Core.Logic.ImageQuantizer.Extensions
{
    ...
    public static partial class Extend
    {
        ...
        public static Int32 GetPaletteColorCount(this Image image)
        {
            ...
            {
                var message =
                    string.Format("Cannot retrieve a color count from a non-indexed image with pixel format '{0}'.",
                                  image.PixelFormat);
        ...
        public static List<Color> GetPalette(this Image image)
        {
            ...
            {
                var message =
                    string.Format("Cannot retrieve a palette from a non-indexed image with pixel format '{0}'.",
                                  image.PixelFormat);
                throw new InvalidOperationException(message);
            }
        ...
        public static void SetPalette(this Image image, List<Color> palette)
        {
            ...
            if (!image.PixelFormat.IsIndexed())
            {
                var message = string.Format(
                    "Cannot store a palette to a non-indexed image with pixel format '{0}'.", image.PixelFormat);
                throw new InvalidOperationException(message);
            }
            ...
            if (palette.Count > imagePalette.Entries.Length)
            {
                var message =
                    string.Format(
                        "Cannot store a palette with '{0}' colors intto an image palette where only '{1}' colors are allowed.",
                        palette.Count, imagePalette.Entries.Length);
