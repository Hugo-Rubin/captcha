...
namespace Core.Logic.ImageQuantizer.Ditherers
{
    public abstract class BaseColorDitherer : IColorDitherer
    {
        ...
        private Int32 GetMatrixFactor()
        {
            ...
            for (var y = 0; y < CachedMatrix.GetLength(0); y++)
                for (var x = 0; x < CachedMatrix.GetLength(1); x++)
                {
        ...
        private IPathProvider GetPathProvider()
        {
            ...
            var result = pathProvider ?? (pathProvider = OnCreateDefaultPathProvider());
            ...
            if (result == null)
            {
                var message =
                    string.Format(
                        "The path provider is not initialized! Please use SetPathProvider() method on quantizer.");
        ...
        protected virtual void OnPrepare()
        {
            ...
            var width = CachedMatrix.GetLength(1);
            var height = CachedMatrix.GetLength(0);
        ...
        /// </summary>
        public void Prepare(IColorQuantizer quantizer, Int32 colorCount, ImageBuffer sourceBuffer,
