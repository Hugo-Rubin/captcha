...
namespace Core.Logic.ImageQuantizer.Quantizers.DistinctSelection
{
    ...
    public class DistinctSelectionQuantizer : BaseColorCacheQuantizer
    {
        ...
        private static Boolean ProcessList(Int32 colorCount, List<DistinctColorInfo> list,
                                           ICollection<IEqualityComparer<DistinctColorInfo>> comparers,
                                           out List<DistinctColorInfo> outputList)
        {
        ...
        protected override List<Color> OnGetPaletteToCache(Int32 colorCount)
        {
...
#if (UseDictionary)
            var colorInfoList = colorMap.Values.ToList();
#else
            ...
            if (foundColorCount >= colorCount)
            {
                ...
                var hueComparer = new ColorHueComparer();
                var saturationComparer = new ColorSaturationComparer();
                var brightnessComparer = new ColorBrightnessComparer();
                ...
                var listColorCount = colorInfoList.Count();

                if (listColorCount > 0)
                {
                    var allowedTake = Math.Min(colorCount, listColorCount);
        ...
        {
            #region IEqualityComparer<DistinctColorInfo> Members
