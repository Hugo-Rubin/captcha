...
namespace Core.Logic.ImageQuantizer.Quantizers.MedianCut
{
    internal class MedianCutCube
    {
        ...
        public Color Color
        {
            get
            {
                ...
                foreach (var color in colorList.Select(Color.FromArgb))
                {
        ...
        private void Shrink()
        {
            ...
            foreach (var argb in colorList)
            {
                var color = Color.FromArgb(argb);

                var red = ColorModelHelper.GetComponentA(ColorModel, color);
                var green = ColorModelHelper.GetComponentB(ColorModel, color);
                var blue = ColorModelHelper.GetComponentC(ColorModel, color);
        ...
        public void SplitAtMedian(Byte componentIndex, out MedianCutCube firstMedianCutCube,
                                  out MedianCutCube secondMedianCutCube)
        {
            List<Int32> colors;
            ...
            switch (componentIndex)
            {
                ...
                case 0:
                    colors =
                        colorList.OrderBy(argb => ColorModelHelper.GetComponentA(ColorModel, Color.FromArgb(argb))).
                            ToList();
                    break;
                ...
                case 1:
                    colors =
                        colorList.OrderBy(argb => ColorModelHelper.GetComponentB(ColorModel, Color.FromArgb(argb))).
                            ToList();
                    break;
                ...
                case 2:
                    colors =
                        colorList.OrderBy(argb => ColorModelHelper.GetComponentC(ColorModel, Color.FromArgb(argb))).
                            ToList();
                    break;
            ...
            var medianIndex = colorList.Count >> 1;
