...
namespace Core.Logic.ImageQuantizer.Quantizers.Uniform
{
    ///<summary>
    ...
    public class UniformQuantizer : BaseColorQuantizer
    {
        ...
        protected override List<Color> OnGetPalette(Int32 colorCount)
        {
            ...
            var optimizedPalette = base.OnGetPalette(colorCount);
            ...
            foreach (var redSlot in redSlots)
                foreach (var greenSlot in greenSlots)
                    foreach (var blueSlot in blueSlots)
                    {
                        var red = redSlot.GetAverage();
                        var green = greenSlot.GetAverage();
                        var blue = blueSlot.GetAverage();
