...
namespace Core.Logic.ImageQuantizer.Quantizers.Popularity
{
    ///<summary>
    ...
    public class PopularityQuantizer : BaseColorCacheQuantizer
    {
        ...
        private static Int32 GetColorIndex(Color color)
        {
            ...
            var redIndex = color.R >> 2;
            var greenIndex = color.G >> 2;
            var blueIndex = color.B >> 2;
        ...
        protected override void OnAddColor(Color color, Int32 key, Int32 x, Int32 y)
        {
            ...
            colorMap.AddOrUpdate(index, colorKey => new PopularityColorSlot(color),
                                 (colorKey, slot) => slot.AddValue(color));
        }
