...
namespace Core.Logic.ImageQuantizer.Helpers
{
    public class ColorModelHelper
    {
        ...
        public static Color HSBtoRGB(Single hue, Single saturation, Single brightness)
        {
            ...
            if (brightness > 0.0f)
            {
                ...
                else // the color is more complex
                {
                    ...
                    var factorHue = hue / 360.0f;
                    var factorA = brightness < 0.5f
                                         ? brightness * (1.0f + saturation)
                                         : (brightness + saturation) - (brightness * saturation);
                    var factorB = (2.0f * brightness) - factorA;
            ...
            var argb = 255 << 24 | red << 16 | green << 8 | blue;
        ...
        public static void RGBtoXYZ(Int32 red, Int32 green, Int32 blue, out Single x, out Single y, out Single z)
        {
            ...
            var redFactor = red / 255.0;
            var greenFactor = green / 255.0;
            var blueFactor = blue / 255.0;

            // convert to a sRGB form
            var sRed = (redFactor > 0.04045) ? Math.Pow((redFactor + 0.055) / (1 + 0.055), 2.2) : (redFactor / 12.92);
            var sGreen = (greenFactor > 0.04045)
                                ? Math.Pow((greenFactor + 0.055) / (1 + 0.055), 2.2)
                                : (greenFactor / 12.92);
            var sBlue = (blueFactor > 0.04045) ? Math.Pow((blueFactor + 0.055) / (1 + 0.055), 2.2) : (blueFactor / 12.92);
        ...
        public static Int32 GetEuclideanDistance(Color color, ColorModel colorModel, IList<Color> palette)
        {
            ...
            var leastDistance = Int64.MaxValue;
            var result = 0;

            for (var index = 0; index < palette.Count; index++)
            {
                var targetColor = palette[index];
                var distance = GetColorEuclideanDistance(colorModel, color, targetColor);
        ...
        public static void GetColorComponents(ColorModel colorModel, Color color, out Single componentA,
                                              out Single componentB, out Single componentC)
        {
            componentA = 0.0f;
            componentB = 0.0f;
            componentC = 0.0f;
        ...
        public static void GetColorComponents(ColorModel colorModel, Color color, Color targetColor,
                                              out Single componentA, out Single componentB, out Single componentC)
        {
            componentA = 0.0f;
            componentB = 0.0f;
            componentC = 0.0f;
