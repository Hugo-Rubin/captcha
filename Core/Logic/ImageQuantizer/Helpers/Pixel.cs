...
using Core.Logic.ImageQuantizer.Helpers.Pixels;
using Core.Logic.ImageQuantizer.Helpers.Pixels.Indexed;
using Core.Logic.ImageQuantizer.Helpers.Pixels.NonIndexed;
...
namespace Core.Logic.ImageQuantizer.Helpers
{
    ...
    public class Pixel : IDisposable
    {
        ...
        private void Initialize()
        {
            ...
            var newType = Expression.New(pixelType);
            var convertNewType = Expression.Convert(newType, typeof(Object));
            var indexedExpression = Expression.Lambda<Func<Object>>(convertNewType);
        ...
        internal Type GetIndexedType(PixelFormat pixelFormat)
        {
            switch (pixelFormat)
            {
                ...
                default:
                    var message = String.Format("This pixel format '{0}' is either non-indexed, or not supported.",
                                                   pixelFormat);
        ...
        internal Type GetNonIndexedType(PixelFormat pixelFormat)
        {
            switch (pixelFormat)
            {
                ...
                default:
                    var message = String.Format("This pixel format '{0}' is either indexed, or not supported.",
                                                   pixelFormat);
                    throw new NotSupportedException(message);
            }
        }
