...
using Core.Logic.ImageQuantizer.Extensions;
using Core.Logic.ImageQuantizer.PathProviders;
...
namespace Core.Logic.ImageQuantizer.Helpers
{
    public class ImageBuffer : IDisposable
    {
        ...
        public delegate Boolean TransformPixelAdvancedFunction(
            Pixel sourcePixel, Pixel targetPixel, ImageBuffer sourceBuffer, ImageBuffer targetBuffer);
        ...
        public void ReadPixel(Pixel pixel, Byte[] buffer = null)
        {
            ...
            var offset = fastByteX[pixel.X] + fastY[pixel.Y];
        ...
        public Int32 GetIndexFromPixel(Pixel pixel)
        {
            ...
            else // not possible to get index from a non-indexed format
            {
                var message =
                    string.Format(
                        "Cannot retrieve index for a non-indexed format. Please use Color (or Value) property instead.");
        ...
        public void SetIndexToPixel(Pixel pixel, Int32 index, Byte[] buffer = null)
        {
            ...
            else // cannot write color to an indexed format
            {
                var message =
                    string.Format(
                        "Cannot set index for a non-indexed format. Please use Color (or Value) property instead.");
                throw new NotSupportedException(message);
            }
        }
        ...
        public void SetColorToPixel(Pixel pixel, Color color, IColorQuantizer quantizer)
        {
            ...
            if (pixel.IsIndexed)
            {
                ...
                if (quantizer != null)
                {
                    var index = (Byte)quantizer.GetPaletteIndex(color, pixel.X, pixel.Y);
                    pixel.Index = index;
                }
                else // cannot write color to an index format
                {
                    var message =
                        string.Format("Cannot retrieve color for an indexed format. Use GetPixelIndex() instead.");
                    throw new NotSupportedException(message);
                }
            }
        ...
        public void WriteColorUsingPixelAt(Pixel pixel, Int32 x, Int32 y, Color color, IColorQuantizer quantizer,
                                           Byte[] buffer = null)
        {
            // redirects pixel -> [x, y]
            pixel.Update(x, y);
        ...
        private void ProcessInParallel(ICollection<Point> path, Action<LineTask> process, Int32 parallelTaskCount = 4)
        {
            ...
            var pointsPerTask = (1.0 * path.Count) / parallelTaskCount;
            var lineTasks = new LineTask[parallelTaskCount];
            var pointOffset = 0.0;

            // creates task for each batch of rows
            for (var index = 0; index < parallelTaskCount; index++)
            {
        ...
        private void ProcessPerPixelBase(IList<Point> path, Delegate processingAction, Int32 parallelTaskCount = 4)
        {
            ...
            // determines mode
            var isAdvanced = processingAction is ProcessPixelAdvancedFunction;

            // prepares the per pixel task
            Action<LineTask> processPerPixel = lineTask =>
                                                   {
                                                       // initializes variables per task
                                                       var pixel = new Pixel(this);

                                                       for (var pathOffset = lineTask.StartOffset;
                                                            pathOffset < lineTask.EndOffset;
                                                            pathOffset++)
                                                       {
                                                           ...
                                                           if (isAdvanced)
                                                           {
                                                               var processAdvancedFunction =
                                                                   (ProcessPixelAdvancedFunction)processingAction;
                                                               allowWrite = processAdvancedFunction(pixel, this);
                                                           }
                                                           else // use simplified version with pixel parameter only
                                                           {
                                                               var processFunction =
                                                                   (ProcessPixelFunction)processingAction;
        ...
        public void ProcessPerPixel(IList<Point> path, ProcessPixelFunction processPixelFunction,
                                    Int32 parallelTaskCount = 4)
        {
            ProcessPerPixelBase(path, processPixelFunction, parallelTaskCount);
        }

        public void ProcessPerPixelAdvanced(IList<Point> path, ProcessPixelAdvancedFunction processPixelAdvancedFunction,
                                            Int32 parallelTaskCount = 4)
        {
        ...
        private void TransformPerPixelBase(ImageBuffer target, IList<Point> path, Delegate transformAction,
                                           Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(path, "path");
            ...
            // determines mode
            var isAdvanced = transformAction is TransformPixelAdvancedFunction;

            // process the image in a parallel manner
            Action<LineTask> transformPerPixel = lineTask =>
                                                     {
                                                         // creates individual pixel structures per task
                                                         var sourcePixel = new Pixel(this);
                                                         var targetPixel = new Pixel(target);
                                                              ...
                                                              pathOffset++)
                                                         {
                                                             ...
                                                             if (isAdvanced)
                                                             {
                                                                 var transformAdvancedFunction =
                                                                     (TransformPixelAdvancedFunction)transformAction;
                                                                 allowWrite = transformAdvancedFunction(sourcePixel,
                                                                                                        targetPixel,
                                                                                                        this, target);
                                                             }
                                                             else // use simplified version with pixel parameters only
                                                             {
                                                                 var transformFunction =
                                                                     (TransformPixelFunction)transformAction;
        ...
        public void TransformPerPixel(ImageBuffer target, IList<Point> path,
                                      TransformPixelFunction transformPixelFunction, Int32 parallelTaskCount = 4)
        {
            TransformPerPixelBase(target, path, transformPixelFunction, parallelTaskCount);
        }

        public void TransformPerPixelAdvanced(ImageBuffer target, IList<Point> path,
        ...
        public static void ScanImageColors(Image sourceImage, IColorQuantizer quantizer, Int32 parallelTaskCount = 4)
        {
            ...
            using (var source = new ImageBuffer(sourceImage, ImageLockMode.ReadOnly))
            {
        ...
        public static List<Color> SynthetizeImagePalette(Image sourceImage, IColorQuantizer quantizer, Int32 colorCount,
                                                         Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
        ...
        public void Quantize(ImageBuffer target, IColorQuantizer quantizer, Int32 colorCount,
                             Int32 parallelTaskCount = 4)
        {
        ...
        public void Quantize(ImageBuffer target, IColorQuantizer quantizer, IColorDitherer ditherer, Int32 colorCount,
                             Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(target, "target");
            ...
            var isTargetIndexed = target.PixelFormat.IsIndexed();

            // step 1 - prepares the palettes
            var targetPalette = isTargetIndexed
                                            ? SynthetizePalette(quantizer, colorCount, parallelTaskCount)
                                            : null;
        ...
        public static Image QuantizeImage(ImageBuffer source, IColorQuantizer quantizer, Int32 colorCount,
                                          Int32 parallelTaskCount = 4)
        {
            // performs the pure quantization wihout dithering
        ...
        public static Image QuantizeImage(ImageBuffer source, IColorQuantizer quantizer, IColorDitherer ditherer,
                                          Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            ...
            using (var target = new ImageBuffer(result, lockMode))
            {
        ...
        public static Image QuantizeImage(Image sourceImage, IColorQuantizer quantizer, Int32 colorCount,
                                          Int32 parallelTaskCount = 4)
        {
            // performs the pure quantization wihout dithering
        ...
        public static Image QuantizeImage(Image sourceImage, IColorQuantizer quantizer, IColorDitherer ditherer,
                                          Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
            ...
            // wraps source image to a buffer
            using (var source = new ImageBuffer(sourceImage, lockMode))
            {
            ...
            TransformPixelFunction calculateMeanError = (sourcePixel, targetPixel) =>
                                                            {
                                                                ...
                                                                totalError +=
                                                                    ColorModelHelper.GetColorEuclideanDistance(
                                                                        ColorModel.RedGreenBlue, sourceColor,
                                                                        targetColor);
                                                                return false;
                                                            };
        ...
        public static Double CalculateImageMeanError(ImageBuffer source, Image targetImage, Int32 parallelTaskCount = 4)
        {
            ...
            // wraps source image to a buffer
            using (var target = new ImageBuffer(targetImage, ImageLockMode.ReadOnly))
            {
                // use other override to calculate error
                return source.CalculateMeanError(target, parallelTaskCount);
            }
        ...
        public static Double CalculateImageNormalizedMeanError(ImageBuffer source, Image targetImage,
                                                               Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(source, "source");
        ...
        public static Double CalculateImageNormalizedMeanError(Image sourceImage, ImageBuffer target,
                                                               Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
        ...
        public static Double CalculateImageNormalizedMeanError(ImageBuffer source, ImageBuffer target,
                                                               Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(source, "source");
        ...
        public static Double CalculateImageNormalizedMeanError(Image sourceImage, Image targetImage,
                                                               Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
        ...
        public void ChangeFormat(ImageBuffer target, IColorQuantizer quantizer, Int32 parallelTaskCount = 4)
        {
            ...
            var hasSourceAlpha = PixelFormat.HasAlpha();
            var hasTargetAlpha = target.PixelFormat.HasAlpha();
            var isTargetIndexed = target.PixelFormat.IsIndexed();
            var isSourceDeepColor = PixelFormat.IsDeepColor();
            var isTargetDeepColor = target.PixelFormat.IsDeepColor();
            ...
            TransformPixelFunction changeFormat = (sourcePixel, targetPixel) =>
                                                      {
                                                          ...
                                                          else
                                                          {
                                                              ...
                                                              {
                                                                  var argb = 255 << 24 | color.R << 16 | color.G << 8 |
                                                                               color.B;
        ...
        public static void ChangeFormat(ImageBuffer source, PixelFormat targetFormat, IColorQuantizer quantizer,
                                        out Image targetImage, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(source, "source");
        ...
        public static void ChangeFormat(Image sourceImage, PixelFormat targetFormat, IColorQuantizer quantizer,
                                        out Image targetImage, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
        ...
        public void Dither(ImageBuffer target, IColorDitherer ditherer, IColorQuantizer quantizer, Int32 colorCount,
                           Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(target, "target");
        ...
        public static void DitherImage(ImageBuffer source, ImageBuffer target, IColorDitherer ditherer,
                                       IColorQuantizer quantizer, Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(source, "source");
        ...
        public static void DitherImage(ImageBuffer source, Image targetImage, IColorDitherer ditherer,
                                       IColorQuantizer quantizer, Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(source, "source");
        ...
        public static void DitherImage(Image sourceImage, ImageBuffer target, IColorDitherer ditherer,
                                       IColorQuantizer quantizer, Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
        ...
        public static void DitherImage(Image sourceImage, Image targetImage, IColorDitherer ditherer,
                                       IColorQuantizer quantizer, Int32 colorCount, Int32 parallelTaskCount = 4)
        {
            // checks parameters
            Guard.CheckNull(sourceImage, "sourceImage");
            ...
            ProcessPixelFunction correctGamma = pixel =>
                                                    {
                                                        ...
                                                        var red = gammaRamp[oldColor.R];
                                                        var green = gammaRamp[oldColor.G];
                                                        var blue = gammaRamp[oldColor.B];
        ...
        {
            /// <summary>
            ///   Initializes a new instance of the <see cref="ImageBuffer.LineTask" /> class.
            /// </summary>
